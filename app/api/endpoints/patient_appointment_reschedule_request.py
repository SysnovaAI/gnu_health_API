...
@router.post("/request-appointment-reschedule")
def request_appointment_reschedule(
    ...
    if not patient:
        raise HTTPException(status_code=403, detail="Only patients can request appointment rescheduling")
    ...
    if appointment[1] != "confirmed":
        raise HTTPException(
            status_code=400, 
            detail="Only confirmed appointments can be requested for rescheduling"
        )

    try:
        # Update the appointment state to "reschedule request"
        update_query = text("""
            UPDATE gnuhealth_appointment 
            SET state = 'reschedule request',
        ...
        return {
            "success": True,
            "message": "Appointment reschedule request has been sent to the hospital"
        }
